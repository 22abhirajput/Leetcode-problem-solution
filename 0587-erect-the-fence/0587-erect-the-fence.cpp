// it's an Andrew's monotone chain convex hull algorithm that constructs the convex hull of a set of 2-dimensional points in O(nlogn) time.
class Solution {
public:
    ...
    vector<vector<int>> outerTrees(vector<vector<int>>& trees) {
        ...
        rTrees.pop_back();
        lTrees.pop_back();
        for (int i = 0; i < rTrees.size(); i++) { // concatinting lower hull to upper hull
